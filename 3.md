...
## מימוש ה-Decorator `fair_deck`
הנה דוגמה בסיסית למימוש ה-decorator `fair_deck`:

```python
# Custom Exception להתמודדות עם ניסיונות רמאות
class DeckCheatingError(Exception):
    """חריגה שנזרקת כאשר חפיסת קלפים מוחזרת ללא שינוי בסדר הקלפים."""
    def __init__(self, message="Cheating detected! The deck was not properly shuffled."):
...
def fair_deck(func):
    """
    Decorator שמוודא שפונקציות המחזירות חפיסות קלפים אכן מערבבות אותן כראוי.
    
    הדקורטור מפעיל את הפונקציה פעמיים ובודק שהתוצאות שונות, מה שמבטיח
    שאלגוריתם הערבוב באמת אקראי ולא מחזיר את אותה התוצאה.
    
    Args:
        func: הפונקציה שיש לקשט
        
    Returns:
        החפיסה המעורבבת מההפעלה הראשונה
        
    Raises:
        DeckCheatingError: אם הפונקציה מחזירה את אותה חפיסה בשתי ההפעלות
    """
    
    def wrapper(*args, **kwargs):
        # הפעלה ראשונה של הפונקציה
        first_result = func(*args, **kwargs)
        
        # הפעלה שנייה של הפונקציה (לבדיקה בלבד)
        second_result = func(*args, **kwargs)
        
        # בדיקה אם הסדר של הקלפים זהה בשתי ההפעלות
        if len(first_result) == len(second_result):
            ...
            # אם הסדר זהה, זריקת חריגה
            if same_order:
                raise DeckCheatingError()
        
        # החזרת התוצאה של ההפעלה הראשונה
        return first_result
